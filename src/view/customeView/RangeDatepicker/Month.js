	...
	getDayStack(month) {
		...
		do {
			...
			for (let i = 0; i < 7; i++) {
				...
				if (arr[i] == currDate.days() && currDate.month() == currMonth) {
	...
	render() {
		...
		return (
			<View style ={{backgroundColor:"#f7f7f7"}}>
				<Text style={{ fontSize: 20, padding: 10, textAlign: "center", color: "#77a300" }}>THÁNG {moment(month, 'YYYYMM').format("M  YYYY")}</Text>
				<View>
					{
						dayStack.map((days, i) => {
							return (
